...
contract C4PoC is Test {
    ...
    function testSubmissionValidity() public {
        // ╔═══════════════════════════════════════════════════════════════════╗
        // ║   PoC: Single Invalid Price Causes Global Price Update Revert     ║
        // ╚═══════════════════════════════════════════════════════════════════╝
        
        // 1. Capture the initial prices before our test
        uint256[6] memory initialStates; // WethPrice, WethTime, UsdcPrice, UsdcTime, LinkPrice, LinkTime
        (initialStates[0], initialStates[1], ) = auction.getAssetPrice(address(mockWETH));
        (initialStates[2], initialStates[3], ) = auction.getAssetPrice(address(mockUSDC));
        (initialStates[4], initialStates[5], ) = auction.getAssetPrice(address(mockLINK));

        // 2. Advance time to make current prices clearly distinguishable from new ones
        skip(1000);
        uint256 currentTime = block.timestamp;
        
        // ---------------------------------------------------------------------
        // SCENARIO A: USDC is stale, WETH and LINK are valid
        // ---------------------------------------------------------------------
        bytes[] memory unverifiedReportsA = new bytes[](3);
        ...
        {
            PriceManager.ReportV3 memory wethReportA;
            wethReportA.dataStreamsFeedId = i_mockWETHFeedId;
            wethReportA.price = int192(uint192(4_100e18));
            wethReportA.observationsTimestamp = uint32(currentTime);
            unverifiedReportsA[0] = abi.encode(context, abi.encode(wethReportA), rs, ss, rawVs);

            PriceManager.ReportV3 memory usdcReportA;
            usdcReportA.dataStreamsFeedId = i_mockUSDCFeedId;
            usdcReportA.price = int192(uint192(1e18));
            usdcReportA.observationsTimestamp = uint32(currentTime - 1 hours - 1); // Stale
            unverifiedReportsA[1] = abi.encode(context, abi.encode(usdcReportA), rs, ss, rawVs);

            PriceManager.ReportV3 memory linkReportA;
            linkReportA.dataStreamsFeedId = i_mockLINKFeedId;
            linkReportA.price = int192(uint192(21e18));
            linkReportA.observationsTimestamp = uint32(currentTime);
            unverifiedReportsA[2] = abi.encode(context, abi.encode(linkReportA), rs, ss, rawVs);
        }

        _changePrank(priceAdmin);
        vm.expectRevert(Errors.StaleFeedData.selector);
        auction.transmit(unverifiedReportsA);
        
        // ---------------------------------------------------------------------
        // SCENARIO B: WETH is stale, USDC and LINK are valid
        // ---------------------------------------------------------------------
        bytes[] memory unverifiedReportsB = new bytes[](3);

        {
            PriceManager.ReportV3 memory wethReportB;
            wethReportB.dataStreamsFeedId = i_mockWETHFeedId;
            wethReportB.price = int192(uint192(4_100e18));
            wethReportB.observationsTimestamp = uint32(currentTime - 1 hours - 1); // Stale
            unverifiedReportsB[0] = abi.encode(context, abi.encode(wethReportB), rs, ss, rawVs);

            PriceManager.ReportV3 memory usdcReportB;
            usdcReportB.dataStreamsFeedId = i_mockUSDCFeedId;
            usdcReportB.price = int192(uint192(1.1e18));
            usdcReportB.observationsTimestamp = uint32(currentTime);
            unverifiedReportsB[1] = abi.encode(context, abi.encode(usdcReportB), rs, ss, rawVs);

            PriceManager.ReportV3 memory linkReportB;
            linkReportB.dataStreamsFeedId = i_mockLINKFeedId;
            linkReportB.price = int192(uint192(21e18));
            linkReportB.observationsTimestamp = uint32(currentTime);
            unverifiedReportsB[2] = abi.encode(context, abi.encode(linkReportB), rs, ss, rawVs);
        }

        vm.expectRevert(Errors.StaleFeedData.selector);
        auction.transmit(unverifiedReportsB);

        // ---------------------------------------------------------------------
        // 3. Verify that ALL valid reports in both scenarios were discarded
        // ---------------------------------------------------------------------
        uint256[6] memory finalStates;
        (finalStates[0], finalStates[1], ) = auction.getAssetPrice(address(mockWETH));
        (finalStates[2], finalStates[3], ) = auction.getAssetPrice(address(mockUSDC));
        (finalStates[4], finalStates[5], ) = auction.getAssetPrice(address(mockLINK));
        
        // Assert prices and timestamps are completely unchanged from before the transmits
        assertEq(finalStates[0], initialStates[0], "WETH price should not have updated");
        assertEq(finalStates[1], initialStates[1], "WETH timestamp should not have updated");
        assertEq(finalStates[2], initialStates[2], "USDC price should not have updated");
        assertEq(finalStates[3], initialStates[3], "USDC timestamp should not have updated");
        assertEq(finalStates[4], initialStates[4], "LINK price should not have updated");
        assertEq(finalStates[5], initialStates[5], "LINK timestamp should not have updated");
    }
